...
# To execute test, run
# cmsRun boostedAnalysis_cfg.py isData=False outputFile=testrun
# maxEvents=100
# inputFiles=/store/user/mschrode/ttHTobb_M125_TuneCUETP8M2_ttHtranche3_13TeV-powheg-pythia8/Skim-V1_3j20_1l20/170217_163603/0000/Skim_1.root
...
options.register("outName", "testrun", VarParsing.multiplicity.singleton,
                 VarParsing.varType.string, "name and path of the output files (without extension)")
options.register("weight", 0.01, VarParsing.multiplicity.singleton,
                 VarParsing.varType.float, "xs*lumi/(nPosEvents-nNegEvents)")
...
options.register("isData", False, VarParsing.multiplicity.singleton,
                 VarParsing.varType.bool, "is it data or MC?")
options.register("isBoostedMiniAOD", False, VarParsing.multiplicity.singleton, VarParsing.varType.bool,
                 "has the file been prepared with the BoostedProducer ('custom' MiniAOD)?")
options.register("generatorName", "POWHEG", VarParsing.multiplicity.singleton,
                 VarParsing.varType.string, "'POWHEG','aMC', 'MadGraph' or 'pythia8'")
...
options.register("useJson", False, VarParsing.multiplicity.singleton, VarParsing.varType.bool,
                 "apply the json filter (on the grid there are better ways to do this)")
options.register("additionalSelection", "NONE", VarParsing.multiplicity.singleton,
                 VarParsing.varType.string, "addition Selection to use for this sample")
datasets = ['NA', 'mu', 'el', 'elel', 'elmu', 'mumu']
options.register("dataset", "NA", VarParsing.multiplicity.singleton, VarParsing.varType.string,
                 "flag to identify which dataset is used, can be "+','.join(datasets))
options.register("dumpSyncExe", False, VarParsing.multiplicity.singleton,
                 VarParsing.varType.bool, "Dump textfiles for sync exe?")
options.register("systematicVariations", "nominal", VarParsing.multiplicity.list, VarParsing.varType.string,
                 "comma-separated list of systematic variations ('nominal' or systematics base name, up/down will be added)")
options.register("deterministicSeeds", True, VarParsing.multiplicity.singleton,
                 VarParsing.varType.bool, "create collections with deterministic seeds")
options.register("electronRegression", "GT", VarParsing.multiplicity.singleton, VarParsing.varType.string,
                 "'GT' or an absolute path to a sqlite file for electron energy regression")
options.register("electronSmearing", "Moriond17_23Jan", VarParsing.multiplicity.singleton,
                 VarParsing.varType.string, "correction type for electron energy smearing")
options.register("useMuonRC", True, VarParsing.multiplicity.singleton,
                 VarParsing.varType.bool, "use Rochester Correction for muons")
options.register("recorrectMET",     True,     VarParsing.multiplicity.singleton,
                 VarParsing.varType.bool,     "recorrect MET using latest JES and e/g corrections")
options.register("dataEra",     "",     VarParsing.multiplicity.singleton,
                 VarParsing.varType.string,     "the era of the data taking period, e.g. '2016B', empty for MC")
options.register("updatePUJetId",     True,     VarParsing.multiplicity.singleton,
                 VarParsing.varType.bool,     "update the PUJetId values")
options.register("ProduceMemNtuples", False, VarParsing.multiplicity.singleton,
                 VarParsing.varType.bool, "do you want to produce slimmed ntuples as input to mem code?")
...
if not options.inputFiles:
    if options.isData:
        options.inputFiles = [
            'root://xrootd-cms.infn.it//store/data/Run2016B/MET/MINIAOD/03Feb2017_ver2-v2/100000/028C28AD-47EE-E611-861A-0025905A48EC.root']
...
if options.deterministicSeeds:
    ...
    tauCollection = cms.InputTag(
        "deterministicSeeds", "tausWithSeed", process.name_())
    photonCollection = cms.InputTag(
        "deterministicSeeds", "photonsWithSeed", process.name_())
    jetCollection = cms.InputTag(
        "deterministicSeeds", "jetsWithSeed", process.name_())
    METCollection = cms.InputTag(
        "deterministicSeeds", "METsWithSeed", process.name_())
...
if options.isData:
    process.ak4PFchsL1L2L3.correctors.append(
        'ak4PFchsResidual')  # add residual JEC for data
...
if options.isData:
    process.ak8PFchsL1L2L3.correctors.append(
        'ak8PFchsResidual')  # add residual JEC for data
...
if options.isData:
    process.GlobalTag.toGet.append(
        cms.PSet(
            ...
            tag=cms.string(
                'JetCorrectorParametersCollection_Summer16_23Sep2016AllV4_DATA_AK4PFchs'),
...
if options.electronRegression:
    if options.electronRegression == "GT":
        ...
        GBRDWrapperRcd.connect = cms.string(
            "sqlite_file:" + options.electronRegression)
        process.regressions = GBRDWrapperRcd
        process.regressions.DumpStat = cms.untracked.bool(False)
        process.es_prefer_regressions = cms.ESPrefer(
            "PoolDBESSource", "regressions")
...
if options.electronSmearing and options.electronRegression:
    ...
    process.calibratedPatElectrons.correctionFile = cms.string(
        files[options.electronSmearing])
    ...
    # overwrite output collections
    electronCollection = cms.InputTag(
        "calibratedPatElectrons", "", process.name_())
...
process.SelectedElectronProducer.collectionNames = [
    "selectedElectronsLoose", "selectedElectronsDL", "selectedElectrons"]
...
process.SelectedMuonProducer.collectionNames = [
    "selectedMuonsLoose", "selectedMuonsDL", "selectedMuons"]
...
process.SelectedTauProducer.collectionNames = [
    "selectedTausLoose", "selectedTausDL", "selectedTaus"]
...
if options.recorrectMET:
    ...
    # we only need to overwrite patMultPhiCorrParams_T1Txy_25ns with the new
    # one
    if options.isData:
    ...
    # also add MET corrections due to e/g corrections, such as the slew rate
    # fix in reMiniAOD
    if options.isData:
        ...
        # overwrite output collections
        METCollection = cms.InputTag(
            "slimmedMETsMuEGClean", "", process.name_())
...
if options.updatePUJetId:
    ...
    process.updatedPatJets.userData.userFloats.src.append(
        "pileupJetIdUpdated:fullDiscriminant")
    process.updatedPatJets.userData.userInts.src.append(
...
process.SelectedJetProducer.collectionNames = [
    "selectedJetsLoose", "selectedJets"]
...
for syst in systs:
    setattr(process, 'SelectedJetProducer'+syst, process.SelectedJetProducer.clone(jets='patSmearedJets' +
                                                                                   syst, collectionNames=[n+syst for n in list(process.SelectedJetProducer.collectionNames)]))

# correction of  miniAOD jets -- one producer creates a jet collection for
# nominal JES and every JES systematic
...
process.patSmearedJets = cms.EDProducer("SmearedPATJetProducer",
                                        src=cms.InputTag(
                                            "CorrectedJetProducer:correctedJets"),
                                        # If False, no smearing is performed
                                        enabled=cms.bool(True),
                                        rho=cms.InputTag(
                                            "fixedGridRhoFastjetAll"),
                                        # If True, always skip gen jet matching
                                        # and smear jet with a random gaussian
                                        ...
                                        # systematic +1 0 -1 sigma
                                        variation=cms.int32(0),
                                        debug=cms.untracked.bool(False),
                                        resolutionFile=cms.FileInPath(
                                            "BoostedTTH/BoostedAnalyzer/data/jerfiles/Spring16_25nsV10_MC_PtResolution_AK4PFchs.txt"),
                                        scaleFactorFile=cms.FileInPath(
                                            "BoostedTTH/BoostedAnalyzer/data/jerfiles/Spring16_25nsV10_MC_SF_AK4PFchs.txt"),
                                        )
# up/down jer shift of nominal sample and nominal jer shift of jes
# systematic samples
for s in systsJER:
    v = 0
    if s == 'JERup':
...
for s in systsJES:
    setattr(process, 'patSmearedJets'+s, process.patSmearedJets.clone(variation=0,
                                                                      src=cms.InputTag("CorrectedJetProducer:correctedJets"+s)))
...
process.GenCollectionProducer.collection_name = [
    "CustomGenElectrons", "CustomGenMuons", "CustomGenTaus", "CustomGenJets", "CustomGenJetsLoose"]
process.GenCollectionProducer.collection_type = [
    "Electron", "Muon", "Tau", "Jet", "Jet"]
...
if options.isData:
    ...
    if not options.isBoostedMiniAOD:
        ...
        # Needed to determine tt+x category -- is usually run when producing
        # boosted jets in miniAOD
...
process.BoostedAnalyzer.selectedJets = [cms.InputTag(
    "SelectedJetProducer"+s+":selectedJets"+s) for s in variations]
process.BoostedAnalyzer.selectedJetsLoose = [cms.InputTag(
    "SelectedJetProducer"+s+":selectedJetsLoose"+s) for s in variations]
process.BoostedAnalyzer.correctedMETs = [cms.InputTag(
    "CorrectedMETproducer:correctedMET")]*(len(variations))
...
if options.isData and options.useJson:
    ...
    process.source.lumisToProcess = LumiList.LumiList(
        filename='/afs/cern.ch/cms/CAF/CMSCOMM/COMM_DQM/certification/Collisions16/13TeV/ReReco/Final/Cert_271036-284044_13TeV_23Sep2016ReReco_Collisions16_JSON.txt').getVLuminosityBlockRange()
...
process.p *= process.regressionApplication*process.selectedElectrons*process.calibratedPatElectrons * \
    process.SelectedElectronProducer*process.SelectedMuonProducer * \
    process.SelectedTauProducer*process.GenCollectionProducer
if options.updatePUJetId:
    process.p *= process.pileupJetIdUpdated*process.updatedPatJets
process.p *= process.CorrectedJetProducer
# always produce (but not necessarily write to ntuple) nominal case as
# collections might be needed
for s in [""]+systs:
